...
Select	  Cast(SubString(MI.Col001,1,8) As VarChar(8)) As [HeaderCode]
		, Cast(SubString(MI.Col001,9,12) As VarChar(8)) As [SendingEntity]
		, Cast(SubString(MI.Col001,13,8) As VarChar(8)) As [FileCreationDate]
		, Cast(SubString(MI.Col001,21,680) As VarChar(8000)) As [Filler01]
		, '' As [Filler02]
...
Select	  Cast(SubString(MI.Col001,1,3) As VarChar(3)) As RecordType
		, Cast(SubString(MI.Col001,4,12) As VarChar(12)) As HICN
...
Left	Outer Join [dbo].[tbl_MSPCOBMA_Primary] As MP
		On MP.HICN=SubString(MI.Col001,4,12)
...
Select	  Cast(SubString(MI.Col001,1,3) As VarChar(3)) As RecordType
		, Cast(SubString(MI.Col001,4,2) As VarChar(2)) As MSPOccurence
		, Cast(SubString(MI.Col001,6,12) As VarChar(12)) As HICN
		...
		, Cast(SubString(MI.Col001,47,4) As VarChar(4)) As [Filler01]
...
Left	Outer Join [dbo].[tbl_MSPCOBMA_Detail] As MD
		On MD.MSPOccurence=SubString(MI.Col001,4,2) 
		And MD.HICN=SubString(MI.Col001,6,12) 
